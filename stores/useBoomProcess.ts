...
export const useCargaInsumosProcessStore = defineStore("cargaInsumosProcess", {
  ...
  actions: {
    ...
    updateExistenciasData(data: any[], fileName = "") {
      ...
      console.log(`📦 BOOM Store: Existencias actualizadas - ${data.length} registros`);
    ...
    updateCoberturaData(data: any[], fileName = "") {
      ...
      console.log(`🛡️ BOOM Store: Cobertura actualizada - ${data.length} registros`);
    ...
    clearStepData(step: 'planVentas' | 'existencias' | 'cobertura') {
      ...
      console.log(`🧹 BOOM Store: Datos de ${step} limpiados`);
    },
    ...
    clearAllData() {
      ...
      console.log("🧹 BOOM Store: Todos los datos limpiados");
    },
    ...
    async processDocuments() {
      if (!this.allStepsHaveData || this.isProcessing) {
        console.warn("⚠️ BOOM Store: No se puede procesar - datos faltantes o ya procesando");
      ...
      try {
        console.log("🚀 BOOM Store: Iniciando procesamiento de documentos...");

        // Simular procesamiento de cada paso
        const steps = ['planVentas', 'existencias', 'cobertura'] as const;

        for (const step of steps) {
          console.log(`⏳ BOOM Store: Procesando ${step}...`);
        ...
        console.log("✅ BOOM Store: Documentos procesados exitosamente");
        ...
        console.error("❌ BOOM Store: Error al procesar documentos:", error);
    ...
    selectDocument(documentId: string) {
      ...
      if (document) {
        ...
        console.log(`📄 BOOM Store: Documento seleccionado - ${document.name}`);
        return document;
      }

      console.warn(`⚠️ BOOM Store: Documento no encontrado - ID: ${documentId}`);
      return null;
    },
    ...
    loadDocumentData(documentId: string) {
      ...
      if (document) {
        ...
        console.log(`📥 BOOM Store: Datos del documento cargados - ${document.name}`);
        return true;
      }
    ...
    deleteDocument(documentId: string) {
      ...
      if (index !== -1) {
        ...
        console.log(`🗑️ BOOM Store: Documento eliminado - ${document.name}`);
        return true;
      }
    ...
    updateDocumentStatus(documentId: string, status: CargaInsumosDocument['status']) {
      ...
      if (document) {
        ...
        console.log(`📄 BOOM Store: Estado del documento actualizado - ${document.name}: ${status}`);
        return true;
      }
    ...
    persistDocuments() {
      try {
        if (typeof window !== 'undefined') {
          localStorage.setItem('boom-documents', JSON.stringify(this.documents));
          console.log("💾 BOOM Store: Documentos persistidos en localStorage");
        }
      } catch (error) {
        console.error("❌ BOOM Store: Error al persistir documentos:", error);
      }
    },
    ...
    loadPersistedDocuments() {
      try {
        if (typeof window !== 'undefined') {
          const stored = localStorage.getItem('boom-documents');
          if (stored) {
            ...
            console.log(`📥 BOOM Store: ${this.documents.length} documentos cargados desde localStorage`);
          }
        }
      } catch (error) {
        console.error("❌ BOOM Store: Error al cargar documentos persistidos:", error);
        this.documents = [];
      }
    },
    ...
    setAutoSave(enabled: boolean) {
      ...
      console.log(`⚙️ BOOM Store: Auto-guardado ${enabled ? 'habilitado' : 'deshabilitado'}`);
    },
    ...
    setMaxDocuments(max: number) {
      ...
      console.log(`⚙️ BOOM Store: Límite de documentos establecido en ${max}`);
    },
    ...
    resetStore() {
      ...
      if (typeof window !== 'undefined') {
        localStorage.removeItem('boom-documents');
      }

      console.log("🔄 BOOM Store: Store reiniciado completamente");
    },
     ...
     */
    async initialize() {
      console.log("🏁 BOOM Store: Inicializando store...");

      // Cargar documentos persistidos
      this.loadPersistedDocuments();

      console.log("✅ BOOM Store: Store inicializado exitosamente");
