<template>
  <div class="">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          >
            ...
            Editar Ventana de Ejecución RPA
          </h1>
          <p class="mt-3 text-lg text-gray-600 dark:text-gray-300 ml-16">
            Modifica los datos de la ventana de ejecución
          </p>
        </div>
    ...
    <!-- Estado de error -->
    <div v-else-if="!window" class="text-center py-16">
      ...
      <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-3">Ventana no encontrada</h3>
      <p class="text-gray-600 dark:text-gray-300 mb-8 max-w-md mx-auto">
        La ventana de ejecución que buscas no existe o ha sido eliminada.
      </p>
      <NuxtLink to="/tools/rpa-horarios">
        <button
          type="button"
      ...
      >
        ...
        <div class="bg-gradient-to-r from-cyan-500 to-cyan-600 px-6 py-4">
          <div class="flex items-center">
            <UIcon name="i-heroicons-clock" class="w-6 h-6 text-white mr-3" />
            <h2 class="text-xl font-semibold text-white">Datos de la Ventana</h2>
          </div>
        </div>

        <div class="p-6">
          <!-- Formulario -->
          <form @submit.prevent="updateWindow" class="space-y-6">
            <!-- Nombre (Destacado) -->
            <div class="relative">
              <div
            ...
            <div class="flex justify-end space-x-3 pt-4">
              ...
              >
                ...
                {{ updating ? "Actualizando..." : "Actualizar Ventana" }}
              </button>
            </div>
          </form>
        </div>
...
const windowId = route.params.id;
...
// Meta tags para SEO
useSeoMeta({
  title: "Editar Ventana RPA - Portal Diveco",
  description: "Editar ventana de ejecución de RPA",
});
...
const fetchWindow = async () => {
  loading.value = true;
  try {
    const { data, errors: fetchErrors } = await client.models.RpaExecutionWindow.get({
      id: windowId,
    });

    if (fetchErrors || !data) {
      console.error("Error fetching window:", fetchErrors);
      toast.add({
        title: "Error",
        description: "No se pudo cargar la ventana de ejecución",
        color: "red",
      });
      window.value = null;
      return;
...
const updateWindow = async () => {
  ...
  try {
    const beforeState = { ...window.value };

    const { data, errors: updateErrors } = await client.models.RpaExecutionWindow.update({
      id: windowId,
    ...
    if (updateErrors) {
      console.error("Error updating window:", updateErrors);
      toast.add({
        title: "Error",
        description: "No se pudo actualizar la ventana de ejecución",
        color: "red",
      });
      return;
    }
    ...
    toast.add({
      title: "Éxito",
      description: "Ventana de ejecución actualizada correctamente",
      color: "green",
    });
